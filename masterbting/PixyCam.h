...
private:
  ...
  const int Center = 158;  //(316 / 2) => pixel width
                           //Using a function that we created based on the relation between the height of a block and it's distance.
                           //Based on the height, the function will calculate the dist
  float CalculateDistance(float height);
                           // This function gets a distance, the XPOS of the block, and it's index, and then
                           // calculates the angle, and saves it in the correct index of the angle variable.
  float CalculateAngle(float distance, float x, int n);
